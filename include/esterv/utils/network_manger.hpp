...
class WifiBox : public QObject {
   ...
   public:
    ...
    Q_INVOKABLE void connect() {
        if (auto service = service_.lock()) {
            service->connect(
                [](bool sucesss) { qDebug() << "connect " << sucesss; });
        }
    }
    ...
    Q_INVOKABLE void disconnect() {
        if (auto service = service_.lock()) {
            service->disconnect(
                [](bool sucesss) { qDebug() << "disconnect " << sucesss; });
        }
    }
...
class WifiModel : public QAbstractListModel {
    ...
    QML_NAMED_ELEMENT(WifiModel)
    QML_UNCREATABLE("Use Manager.wifis")

   public:
    enum ModelRoles : uint16_t {
